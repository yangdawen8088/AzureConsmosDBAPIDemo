...
namespace AzureConsmosDBAPIDemo
{
    ...
    class AdvancedSamples
    {
        ...
        /// 此查询返回范围内的所有实体。 请注意，如果您的表包含大量数据，
        /// 则查询可能会很慢或可能会超时。 在这种情况下，请使用分段查询，
        /// 如 PartitionRangeQueryAsync() 示例方法中所示。
        /// 请注意，出于示例的目的，同步调用 ExecuteSimpleQuery 方法。 
        /// 但是，在使用 async/await 模式的实际应用程序中，最佳实践建
        /// 议始终使用异步方法。
        /// </summary>
        /// <param name="table">示例表名</param>
        ...
        /// <summary>
        /// 演示分区扫描，我们正在搜索分区内的所有实体。 请注意，
        /// 这不如范围扫描有效 - 但绝对比全表扫描更有效。 异步 
        /// API 要求用户处理段大小并使用延续令牌返回下一个段。
        /// </summary>
        /// <param name="table">示例表名</param>
        /// <param name="partitionKey">要搜索的分区</param>
        /// <returns>一个任务对象</returns>
        ...
        private static async Task ServiceStatsSample(CloudTableClient tableClient)
        {
            ...
            try
            {
                ...
                Console.WriteLine("    地位： {0}", stats.GeoReplication.Status);
            }
        ...
        private static string GetTableSasUri(CloudTable table, string storedPolicyName = null)
        {
            ...
            else
            {
                ...
                //还可以在 ad-hoc SAS 上指定一些约束，在存储的访问策略上指定其他约束。 但是，必须对其中一个指定约束； 不能在两者上都指定。
